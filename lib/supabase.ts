  ...
  if (!supabaseInstance) {
    ...
    supabaseInstance = createSupabaseClient(LOCKED_SUPABASE_URL, LOCKED_SUPABASE_ANON_KEY, {
      auth: {
        ...
        // Uniquement côté navigateur (pas SSR/Node) — typeof window garantit l'environnement browser.
        // Vérifie aussi typeof acquire === 'function' car Node 22 expose navigator.locks partiellement.
        lock: typeof window !== 'undefined' &&
              typeof (navigator as any)?.locks?.acquire === 'function'
